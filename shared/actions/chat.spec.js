...
describe('Chat', () => {
  it('Global', () => {
    ...
    expect(serverStore.getState().getIn(['chat', 'messages'])).size(2);
    expect(serverStore.getState().getIn(['chat', 'messages', 1, 'text'])).equal('test1');
    expect(clientStore0.getState().getIn(['chat', 'messages', 1, 'text'])).equal('test1');
    expect(clientStore1.getState().getIn(['chat', 'messages', 1, 'text'])).undefined;
    clientStore1.dispatch(loginUserFormRequest('/test', {id: 'test', login: 'User1'}));
    expect(clientStore1.getState().getIn(['chat', 'messages', 1, 'text'])).equal('test1');

    clientStore0.dispatch(client$chatMessageRequest(null, 'GLOBAL', 'test2'));
    expect(serverStore.getState().getIn(['chat', 'messages', 2, 'text'])).equal('test2');
    expect(clientStore0.getState().getIn(['chat', 'messages', 2, 'text'])).equal('test2');
    expect(clientStore1.getState().getIn(['chat', 'messages', 2, 'text'])).equal('test2');
  });
